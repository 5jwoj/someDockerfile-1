...
if [ $CUSTOM_LIST_FILE ]; then
    ...
    if expr "$CUSTOM_LIST_FILE" : 'http.*' &>/dev/null; then
        ...
        if [ $re == "" ]; then
            ...
            wget -O $customListFile https://ghproxy.com/$CUSTOM_LIST_FILE
        fi
        echo "下载完成。"
...
if [ 0"$CUSTOM_SHELL_FILE" = "0" ]; then
    ...
    if expr "$CUSTOM_SHELL_FILE" : 'http.*' &>/dev/null; then
        ...
        if [ $re == "" ]; then
            ...
            wget -O /scripts/docker/shell_script_mod.sh https://ghproxy.com/$CUSTOM_SHELL_FILE
        fi
        echo "下载完成，开始执行..."
        echo "" >>$mergedListFile
...
if [ $RANDOM_DELAY_MAX ]; then
    if [ $RANDOM_DELAY_MAX -ge 1 ]; then
        ...
        sed -i "/jd_bean_sign.js\|jd_blueCoin.js\|jd_joy_reward.js\|jd_joy_steal.js\|jd_joy_feedPets.js\|jd_car.js\|jd_car_exchange.js\|jd_shop_sign.js\|monk_inter_shop_sign.js/!s/node/sleep \$((RANDOM % \$RANDOM_DELAY_MAX)); node/g" $mergedListFile
    fi
else
